...
async function main() {
  ...
  const botC = await prisma.user.upsert({
    where: { email: 'bot_gamma@seed.local' },
    update: {},
    create: { username: 'bot_gamma', email: 'bot_gamma@seed.local', password: 'seed' },
  });
  const botD = await prisma.user.upsert({
    where: { email: 'bot_delta@seed.local' },
    update: {},
    create: { username: 'bot_delta', email: 'bot_delta@seed.local', password: 'seed' },
  });
